...
print("NearFace.detectFace test")
#detectors = ['opencv', 'ssd', 'dlib', 'mtcnn', 'retinaface']
detectors = ['opencv', 'ssd', 'mtcnn', 'retinaface']

for detector in detectors:
	img = NearFace.detectFace("dataset/img11.jpg", detector_backend = detector)
...
res = NearFace.verify(dataset, detector_backend = 'retinaface')
print(res)

print("ssd detector")
res = NearFace.verify(dataset, detector_backend = 'ssd')
print(res)

print("opencv detector")
res = NearFace.verify(dataset, detector_backend = 'opencv')
print(res)

if False:
	print("dlib detector")
	res = NearFace.verify(dataset, detector_backend = 'dlib')
	print(res)

print("mtcnn detector")
res = NearFace.verify(dataset, detector_backend = 'mtcnn')
print(res)
...
df = NearFace.find(img_path = "dataset/img1.jpg", db_path = "dataset"
	#, model_name = 'Dlib'
)
print(df.head())
...
dfs = NearFace.find(img_path = ["dataset/img1.jpg", "dataset/img2.jpg"], db_path = "dataset"
	#, model_name = 'Dlib'
)
print(dfs[0].head())
print(dfs[1].head())
...
resp_obj = NearFace.verify(dataset)
print(resp_obj)
...
resp_obj = NearFace.analyze(dataset)
...
demography = NearFace.analyze(img)
print(demography)

print("-----------------------------------------")

print("Facial analysis test. Passing all to the action")
demography = NearFace.analyze(img, ['age', 'gender', 'race', 'emotion'])
...
demography = NearFace.analyze(img, ['age', 'gender'])
...
for model in models:
	...
	for metric in metrics:
		for instance in dataset:
			...
			resp_obj = NearFace.verify(img1, img2
						, model_name = model
...
race_model = NearFace.build_model("Race")
...
resp_obj = NearFace.analyze("dataset/img1.jpg", models=facial_attribute_models)
print(resp_obj)
...
if False:
	...
	df = NearFace.find(img_path = "dataset/img1.jpg", db_path = "dataset", model_name = "Ensemble")
	print(df.head())
...
if False:
	...
	resp_obj = NearFace.verify(dataset, model_name = "Ensemble")
...
if False:
	...
	df = NearFace.find("dataset/img1.jpg", db_path = "dataset", model_name = 'Ensemble', model = model, enforce_detection=False)
...
if False:
	...
	res = NearFace.verify(dataset, model_name = "Ensemble", model = model)
	print(res)
...
df = NearFace.find(img1, db_path = "dataset")
